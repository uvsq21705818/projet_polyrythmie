...
def open():
    ...
    menu = tk.Toplevel()
    #c = tk.Canvas(screen, height =400, width = 900, bg = "black")
    def fermer():
        menu.destroy()
        move_temps()

    speed = tk.Scale(menu, orient = 'horizontal', from_ = 1, to = 10, tickinterval = 1, length = 200)
    vit = tk.Label(menu, text = 'vitesse')
    button2 = tk.Button(menu, text = 'valider', command = lambda : vitesse.set(speed.get()))
    btn = tk.Button(menu, text = 'fermer', command = fermer)

    v = speed.get()
    #abel = tk.Label(screen, text = vitesse)
    speed.grid(column = 1)
    vit.grid(column = 0)
    button2.grid(column = 3)
    btn.grid(column = 4)
...
butt = tk.Button(root, text = 'Commencer', command =lambda: open())
l = tk.Label(root, textvariable= str(vitesse))
...
screen.grid(column=0, row=0, columnspan = 2)
